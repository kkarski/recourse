...
description: Use when the user asks for a product specification, PRD, requirements document, feature definition, user stories, acceptance criteria, business rules, or describes a feature they want built. Also use when an architect needs answers about business requirements, validation rules, or what success means, or when placement of business rules and acceptance criteria across use cases vs a global change-set section is unclear. The end goal is a spec that an engineering team can turn into working software: every use case must carry its **descriptive main flow** (steps the actor takes, system responses, alternates, postconditions)—not just a one-line blurb. Mechanics (Spectr CLI, IDs, batching) serve that goal and must never displace it; if throughput, command length, or context budget pressure tempts you to drop main flows or reduce a use case to a teaser, stop and capture the flow first. To create or update the Spectr spec, you MUST use the Spectr CLI and follow `references/spectr-cli-for-pm.md` only—do not embed Spectr workflows here. For RuleSpeak phrasing see references/rule-speak-best-practices.md; for rules vs acceptance criteria, including UC vs global placement, see references/rules_vs_acceptance_criteria.md; for terms see references/terms_and_definitions.md.
---
...
6. **Reconcile prior plan/spec content** (if any existed): every user-facing metric, chart, acceptance criterion, and use-case flow from that artifact must either appear in the active Spectr spec (with business-level definition and AC) or be explicitly marked out of scope / deferred. No silent omission.
...
Every non-deprecated use case must contain **all** of the following — otherwise it is **incomplete** and must be filled in before the spec is considered draftable, regardless of how complete BRs and ACs look:
...
If you find yourself writing only a one-paragraph blurb in a use case's `desc`, treat it as a **failure of this skill**: stop, capture the steps from the source (transcript / plan / interview notes), then resume.

The persistence detail of **where** these parts live in the file (narrative `<p>` before trigger, structured `<div>` blocks for actors / preconditions / postconditions) is covered by **`references/spectr-cli-for-pm.md`**; this skill only requires that the **content** above exists per UC.
...
- [ ] **Every named alternate / exception path** (return visit, invalid input, mode toggles, blocked paths, …) is captured as its own short numbered flow, including the system's response and resulting postcondition
...
- [ ] Business rule **phrasing** conforms to `references/rule-speak-best-practices.md`; **rules vs. acceptance criteria** and **ID/traceability** discipline conform to `references/rules_vs_acceptance_criteria.md`
...
- [ ] Each **BR** / **AC** has one canonical placement: a **single** owning use case **or** the **global** change-set section; no duplicate text; **`Related ACs`** in use case prose matches that placement (apply `references/rules_vs_acceptance_criteria.md` § Placement)
