...
- **Every business rule must have a unique, stable ID** of the form `BR N` (e.g. `BR 1`, `BR 2`). IDs are assigned in the order rules are first added and are referenced from acceptance criteria, tests, plan tasks, and architect/engineer questions.
